...
const API_KEY = process.env.NEXT_PUBLIC_WEB3_STORAGE_KEY;
if (!API_KEY) {
  alert('storage key environment variable is required')
}
...
export function makeMetadataFile(res, filename) {
  const blob = new Blob([JSON.stringify(res)], { type: 'application/json' })
  return new File([blob], filename || 'metadata.json', { type: 'application/json' })
}
